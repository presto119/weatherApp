$(document).ready(function(){
    //4fbed62b7f5c5b8d7459422dc655dbbf
    //https://api.openweathermap.org/data/2.5/weather?q=seoul&appid=4fbed62b7f5c5b8d7459422dc655dbbf


    var cityName = [];  //버튼 클릭시 또는 검색바에서 입력 후 엔터를 누를 시, 도시명 데이터를 수집한다.
    console.log(cityName);

    var myAPI = `4fbed62b7f5c5b8d7459422dc655dbbf`;
    ...
    function w_info(){
        ...
        //append(), prepend() 자식을 추가하는 메서드는 기존 자식을 남기는 상태에서 새로운 자식들을 추가 ==> 기존 데이터를 새롭게 돌리고자 반복하여 적용하고자 할 때는 반복문 이전에 초기화가 필요
        ...
        $("#weather ul li").each(function(index){
            ...
            $.ajax({
                ...
                success : function(data){
                    ...
                    //텍스트화 된 날씨(변수명 weather)를 데이터 이미지를 아이콘으로 변경
    ...
    $(document).on("click",".cities button", function(){
        ...
        cityName.unshift(city_txt);  //cityName이라는 전역 변수의 배열 데이터의 첫번째 인덱스에 추출한 텍스트(도시명)를 넣어준다.
        console.log(cityName);
    ...
    navigator.geolocation.getCurrentPosition(function(position){
        ...
        $.ajax({
            ...
            success : function(data){
                ...
                w_info();  //함수문을 호출한다. => 배열데이터 개수에 따라 구조를 구성하고 그 데이터 내용을 넣겠다는 의미
            }
        });
